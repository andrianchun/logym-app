...
export default function GymAIChat({
    ...
    useEffect(() => {
        ...
        setSessions(prev => {
            const existing = prev.find(s => s.origin === 'logy');
            if (existing) {
            ...
            const newSession = { id: 'logy_' + Date.now(), title: 'Coach Logy', origin: 'logy', unread: true, messages: [aiMsg], updatedAt: Date.now() };
            return [newSession, ...prev];
        });
    }, [plateauInsights]);

    // When the panel opens (any trigger — avatar tap, CTA, elsewhere), route straight into
    // the dedicated Logy session if it has something unread; otherwise leave whatever was
    ...
    const doDeleteChat = (id) => {
        ...
        if (deletingSession?.origin === 'logy') lastDeliveredInsightKeyRef.current = null;
    ...
    const handleSaveMemory = (text) => {
        if (!setLogyMemory) return;
        const trimmed = text.trim().slice(0, 160);
        setLogyMemory(prev => (prev || []).includes(trimmed) ? prev : [...(prev || []), trimmed]);
    };
    ...
    const renderMessageContent = (msg, msgIdx) => {
        ...
        if (msg.content.includes(tagStart) && msg.content.includes(tagEnd)) {
            ...
            let jsonStr = msg.content.substring(startIndex + tagStart.length, msg.content.indexOf(tagEnd)).trim();
            ...
            try {
                ...
                textPart += `\n\n[Error: Format program dari AI tidak valid]`;
            }
        }
