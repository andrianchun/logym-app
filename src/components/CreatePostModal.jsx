...
export default function CreatePostModal({ user, onClose, theme, t, initialFiles = [], postDataOverrides = {} }) {
  ...
  const [postData, setPostData] = useState(postDataOverrides);
  ...
  const isTemplate = postData.type === 'template';
  const isRepost = postData.type === 'repost';
  const programData = postData.programData || {};
  ...
  return (
    <div className="fixed inset-0 z-[999] bg-black/70 backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-4 animate-in fade-in duration-300 touch-none overscroll-contain no-swipe">
      <div className={`w-full sm:max-w-xl ${isDark ? 'bg-slate-900/90 backdrop-blur-2xl border border-white/10' : 'bg-white/90 backdrop-blur-2xl border border-black/10'} rounded-t-[2.5rem] sm:rounded-[2.5rem] flex flex-col max-h-[85vh] shadow-2xl animate-in slide-in-from-bottom-10 overscroll-contain`}>
        
        {/* Header */}
        <div className="p-4 sm:p-5 flex items-center justify-between shrink-0 border-b border-black/5 dark:border-white/5">
          <button 
            type="button"
            data-close-modal="true"
            onClick={() => onClose()} 
            className={`p-2 rounded-full ${isDark ? 'bg-white/10 hover:bg-white/20 text-white' : 'bg-black/5 hover:bg-black/10 text-black'} transition-colors`}
          >
            <X size={20} />
          </button>
          ...
          <button 
            ...
            className={`px-5 py-2 rounded-2xl font-black text-sm ${t?.bgAccent || 'bg-[#3b82f6] text-white'} shadow-lg hover:shadow-xl hover:-translate-y-0.5 transition-all active:translate-y-0 disabled:opacity-50 disabled:hover:translate-y-0`}
          >
            {isUploading ? <Loader2 size={18} className="animate-spin" /> : 'Post'}
          </button>
        </div>

        {/* Scrollable Content */}
        <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-4 overscroll-contain touch-pan-y hide-scrollbar">
          ...
          {isTemplate && (
            <ProgramCard 
              ...
              isDark={isDark} 
              t={t || {}} 
              onRemove={() => setPostData(prev => ({ ...prev, type: 'user_post', programData: null }))}
            />
          )}
