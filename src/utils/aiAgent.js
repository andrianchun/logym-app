...
const TRIVIAL_MESSAGE_PATTERNS = [
    ...
    /^(makasih|thanks|thank you|thx|terima kasih)[!.\s]*(bro|coach|logy)?[!.\s]*$/i,
...
- Coach Logy (this chat): can create a brand-new program or edit/progress an existing active one directly from chat requests; personality (santai/galak/serius/custom) is changed in Settings -> Lanjutan.
...
`;
    ...
    return `You are "Coach Logy", an elite AI Personal Trainer built into the LOGYM app.
...
export const summarizeActivePrograms = (programs, activePlanIds) => {
    ...
    plans.forEach(plan => {
        ...
        plan.routines.forEach(r => {
            ...
            const days = daysArr.length ? ` (${daysArr.join(', ')})` : '';
