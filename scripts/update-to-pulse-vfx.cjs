...
let md = `# Panduan Lengkap Prompt AI: Single Hero Frame (1:1) & Video Pulse VFX

> **🌟 Standar Terbaik: Single Hero Frame + Continuous Glowing Cyan Pulse VFX**:
> 1. **🖼️ Gambar Thumbnail (1:1 Single Hero Frame)**:
>    - 1 Subjek Penuh, 1 Wajah Coach yang 100% konsisten, 1 Alat Gym yang presisi.
>    - Efek **Glowing Cyan Wireframe Anatomical Muscle Highlight** pada otot target.
> 2. **🎥 Video AI (1 Frame Full Sinematik + Continuous Bio-Electric Pulse)**:
>    - **Sepanjang durasi video**, efek serat otot *glowing cyan wireframe* tetap aktif menempel di otot.
>    - Efek mengalir dan berdenyut (*animated flowing electric pulse*) secara semi-transparan mengikuti ritme repetisi: redup lembut saat fase turun/stretch, dan memancar terang dengan aliran energi saat puncak kontraksi!
...
## 🎨 2. Cara Eksekusi Single Hero Image + Video Pulse VFX
...
3. **Generate Video AI dengan Animasi Pulse (Kling / Runway / Luma / Haiper)**:
   * **Upload Hasil Gambar Single Hero** sebagai First Frame (*Image-to-Video*).
   * **Copas Prompt 2 (Video dengan Continuous Animated Cyan Pulse VFX)** di bawah.

---

## 📋 3. Daftar Lengkap Prompt Single Hero Image & Video Pulse VFX (31 Latihan)
...
Exercise & Setting:
The coach is performing "${item.name}" with proper gym biomechanics using ${item.equip} in a dark modern luxury gym. Captured at the peak contraction and muscular engagement point matching Image 3.

Anatomical Highlight:
Add a hyper-detailed glowing cyan / electric neon blue anatomical wireframe muscle fiber highlight overlay specifically over the ${item.muscles}, showing intense muscular activation and striations.

Aesthetics & Lighting:
Moody atmospheric dark background with subtle blue/cyan neon gym accents, strong volumetric rim lighting carving muscle contours and vascularity, crystal clear focus, 8k resolution, ultra-photorealistic, 1:1 square canvas.`;

  // 2. VIDEO PROMPT (With Continuous Animated Cyan Pulse VFX)
  let howtoSteps = '';
...
[PERSISTENT ANIMATED MUSCLE HIGHLIGHT VFX]:
Throughout the entire video, a persistent semi-transparent glowing cyan / neon electric blue anatomical wireframe muscle fiber overlay seamlessly hugs and tracks the working target muscles (${item.muscles}). The cyan highlight dynamically flows and pulses like living bio-electric currents—glowing with a subtle flowing energy pulse during the lowering stretch, and surging into an intense radiant luminous pulse with glowing neural energy lines at the peak contraction.
...
Moody atmospheric gym, dramatic volumetric rim lighting highlighting muscle definition and striations, sweat glistens under studio lighting, 8k resolution, 60fps, ultra-realistic motion blur, masterclass fitness production.`;
  ...
  md += `    <p><b>🎯 Otot Target (Glowing Cyan Pulse):</b> ${item.muscles}</p>\n`;
  md += `    <p><b>🏋️‍♂️ Alat:</b> ${item.equip}</p>\n`;
  md += `  </div>\n`;
  md += `</div>\n\n`;
  md += `**🖼️ Prompt Image (Single Hero 1:1 - Wireframe Cyan):**\n`;
  md += `\`\`\`text\n${promptImage}\n\`\`\`\n\n`;
  md += `**🎥 Prompt Video (Single-Shot + Continuous Animated Cyan Pulse VFX):**\n`;
  md += `\`\`\`text\n${promptVideo}\n\`\`\`\n\n---\n\n`;
...
================================================================================
1. PROMPT IMAGE (SINGLE HERO 1:1 SQUARE - CYAN WIREFRAME HIGHLIGHT)
Upload to ChatGPT: 00_coach_reference.jpg, 0_start_pose.jpg / 1_end_pose.jpg
================================================================================
${promptImage}

================================================================================
2. PROMPT VIDEO (SINGLE SHOT FULL-FRAME + CONTINUOUS ANIMATED CYAN PULSE VFX)
...
console.log('Successfully updated all prompt guides with Continuous Animated Cyan Pulse VFX!');
